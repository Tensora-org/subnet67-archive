...
interface ICrowdloan {
     ...
     */
    function getCrowdloan(uint32 crowdloanId) external view returns (CrowdloanInfo memory);
     ...
     */
    function getContribution(uint32 crowdloanId, bytes32 coldkey) external view returns (uint64);
     ...
     */
    function create(uint64 deposit, uint64 minContribution, uint64 cap, uint32 end, address targetAddress)
        external
        payable;
     ...
     */
    function updateMinContribution(uint32 crowdloanId, uint64 newMinContribution) external payable;
